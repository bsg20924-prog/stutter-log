...
const SYSTEM_INSTRUCTION = [
  ...
  '4. 주어진 상황 목록 중 그 단어가 가장 자연스럽게 들어갈 상황 하나를 고른다(id 그대로).',
  '5. prompt 는 그 상황에서 **상대가 먼저 건네는 말**이다. 예시를 복사하지 말고 매번 다르게 쓴다.',
  '   같은 상황이 여러 번 나와도 멘트가 겹치지 않게 표현을 바꾼다.',
  '6. prompt 와 text 모두 짧게. 각각 한 문장, 25자 이내.',
  '7. prompt 는 답변 문장과 자연스럽게 이어져야 한다(질문에 답이 되게).',
  '8. 조사를 정확히 쓴다(받침 유무에 따라 은/는, 이/가, 을/를).',
  '9. 어색하면 억지로 끼워 넣지 말고, 그 단어가 주인공이 되는 자연스러운 대화를 만든다.',
  '10. 영어 단어는 한국어 문장 안에 그대로 둔다.',
  '11. 존댓말/반말은 상황에 맞춘다(가족은 반말, 친구/동기는 반말, 나머지는 존댓말).',
  '',
  ...
  try {
    const res = await fetch(ENDPOINT, {
      ...
      body: JSON.stringify({
        ...
        generation_config: {
          temperature: 0.9,
          // 짧은 한국어 문장을 만드는 데 추론은 필요 없다. 기본값(medium)이면
          // 응답이 수십 초까지 늘어지고, 그동안 사용자는 4단계에서 기다리게 된다.
